...
SOR_CONFIGS = [
    ("FluidTrap_s100_sor_1_1", "SOR (1×1)", 1, "1×1", "-",  "#2196F3"),
    ("FluidTrap_s100_sor_2_2", "SOR (2×2)", 4, "2×2", "--", "#FF9800"),
    ("FluidTrap_s100_sor_4_1", "SOR (4×1)", 4, "4×1", ":",  "#9C27B0"),
]

CG_CONFIGS = [
    ("FluidTrap_s100_cg_1_1", "CG (1×1)", 1, "1×1", "-",  "#4CAF50"),
    ("FluidTrap_s100_cg_2_2", "CG (2×2)", 4, "2×2", "--", "#F44336"),
    ("FluidTrap_s100_cg_4_1", "CG (4×1)", 4, "4×1", ":",  "#009688"),
]
...
# ---------------------------------------------------------------------------
# Plot 1: Solver iterations per timestep
# ---------------------------------------------------------------------------
...
iters_plot(SOR_CONFIGS,
           "SOR iterations/step — Fluid Trap (100×50)",
           "plot_fts100_sor_iters.pdf")
iters_plot(CG_CONFIGS,
           "CG iterations/step — Fluid Trap (100×50)",
           "plot_fts100_cg_iters.pdf")
...
iters_plot(combined_1_1,
           "SOR vs CG iterations/step — Fluid Trap 100×50 (1×1)",
           "plot_fts100_sor_vs_cg_1_1.pdf")
...
ax.set_title("Wall time: SOR vs CG — Fluid Trap (100×50)", fontsize=14, fontweight="bold")
...
plt.tight_layout()
for dest in [BASE, DOWNLOADS]:
    plt.savefig(os.path.join(dest, "plot_fts100_walltime.pdf"), dpi=150)
print("  Saved: plot_fts100_walltime.pdf")
plt.close()
...
ax.set_title("Strong Scaling Speedup — Fluid Trap (100×50)", fontsize=14, fontweight="bold")
...
plt.tight_layout()
for dest in [BASE, DOWNLOADS]:
    plt.savefig(os.path.join(dest, "plot_fts100_speedup.pdf"), dpi=150)
print("  Saved: plot_fts100_speedup.pdf")
plt.close()
...
print("\n=== Summary Table ===")
print(f"{'Config':<30} {'Wall [s]':>10} {'Speedup':>10} {'Avg iters':>12} {'Max iters':>12}")
print("-" * 77)
for configs, baseline in [(SOR_CONFIGS, baseline_sor), (CG_CONFIGS, baseline_cg)]:
    for stem, label, *_ in configs:
        ...
        print(f"  {label:<28} {wt_str:>10} {su:>10} {avg:>12} {mx:>12}")
    print()
