...
SOR_CONFIGS = [
    ("FluidTrap_s100_sor_1_1", "SOR (1×1)", 1, "1×1", "-",  "#90CAF9"),
    ("FluidTrap_s100_sor_2_2", "SOR (2×2)", 4, "2×2", "--", "#1E88E5"),
    ("FluidTrap_s100_sor_4_1", "SOR (4×1)", 4, "4×1", ":",  "#1565C0"),
]

CG_CONFIGS = [
    ("FluidTrap_s100_cg_1_1", "CG (1×1)", 1, "1×1", "-",  "#A5D6A7"),
    ("FluidTrap_s100_cg_2_2", "CG (2×2)", 4, "2×2", "--", "#43A047"),
    ("FluidTrap_s100_cg_4_1", "CG (4×1)", 4, "4×1", ":",  "#1B5E20"),
]

PCG_CONFIGS = [
    ("FluidTrap_s100_pcg_1_1", "PCG (1×1)", 1, "1×1", "-",  "#CE93D8"),
    ("FluidTrap_s100_pcg_2_2", "PCG (2×2)", 4, "2×2", "--", "#8E24AA"),
    ("FluidTrap_s100_pcg_4_1", "PCG (4×1)", 4, "4×1", ":",  "#4A148C"),
]
...
iters_plot(SOR_CONFIGS, "SOR iterations/step — Fluid Trap (100×50)",      "plot_fts100_sor_iters.pdf")
iters_plot(CG_CONFIGS,  "CG iterations/step — Fluid Trap (100×50)",        "plot_fts100_cg_iters.pdf")
iters_plot(PCG_CONFIGS, "PCG_SSOR iterations/step — Fluid Trap (100×50)", "plot_fts100_pcg_iters.pdf")

iters_plot(
    [("FluidTrap_s100_sor_1_1", "SOR (1×1)", 1, "1×1", "-", "#1E88E5"),
     ("FluidTrap_s100_cg_1_1",  "CG (1×1)",  1, "1×1", "-", "#43A047"),
     ("FluidTrap_s100_pcg_1_1", "PCG (1×1)", 1, "1×1", "-", "#8E24AA")],
    "SOR vs CG vs PCG — iterations/step (1×1, Fluid Trap 100×50)",
    "plot_fts100_all_solvers_1_1.pdf")

iters_plot(
    [("FluidTrap_s100_sor_2_2", "SOR (2×2)", 4, "2×2", "-", "#1E88E5"),
     ("FluidTrap_s100_cg_2_2",  "CG (2×2)",  4, "2×2", "-", "#43A047"),
     ("FluidTrap_s100_pcg_2_2", "PCG (2×2)", 4, "2×2", "-", "#8E24AA")],
    "SOR vs CG vs PCG — iterations/step (2×2, Fluid Trap 100×50)",
    "plot_fts100_all_solvers_2_2.pdf")

iters_plot(
    [("FluidTrap_s100_sor_4_1", "SOR (4×1)", 4, "4×1", "-", "#1E88E5"),
     ("FluidTrap_s100_cg_4_1",  "CG (4×1)",  4, "4×1", "-", "#43A047"),
     ("FluidTrap_s100_pcg_4_1", "PCG (4×1)", 4, "4×1", "-", "#8E24AA")],
    "SOR vs CG vs PCG — iterations/step (4×1, Fluid Trap 100×50)",
    "plot_fts100_all_solvers_4_1.pdf")
...
if all_vals:
    ...
    ax.set_title("Wall time: SOR vs CG vs PCG_SSOR — Fluid Trap (100×50)",
                 fontsize=14, fontweight="bold")
    ax.legend(fontsize=11)
    ax.grid(True, axis="y", linestyle="--", alpha=0.35)
    ax.set_ylim(top=mx * 1.18)
    plt.tight_layout()
    save(fig, "plot_fts100_walltime.pdf")
    plt.close()
...
baseline_sor = times.get("FluidTrap_s100_sor_1_1")
baseline_cg  = times.get("FluidTrap_s100_cg_1_1")
baseline_pcg = times.get("FluidTrap_s100_pcg_1_1")
...
if any([baseline_sor, baseline_cg, baseline_pcg]):
    ...
    ax.set_title("Strong Scaling Speedup — Fluid Trap (100×50)",
                 fontsize=14, fontweight="bold")
    ...
    ax.set_ylim(bottom=0)
    plt.tight_layout()
    save(fig, "plot_fts100_speedup.pdf")
    plt.close()
...
print("\n=== Summary Table — Fluid Trap (100×50) ===")
print(f"{'Config':<22} {'Wall [s]':>10} {'Speedup':>10} {'Avg iters':>12} {'Max iters':>12}")
print("-" * 69)
for configs, baseline in [(SOR_CONFIGS, baseline_sor), (CG_CONFIGS, baseline_cg), (PCG_CONFIGS, baseline_pcg)]:
    for stem, label, *_ in configs:
        ...
        print(f"  {label:<20} {wt_str:>10} {su:>10} {avg:>12} {mx_str:>12}")
    print()
