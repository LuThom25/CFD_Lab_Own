...
Case::Case(std::string file_name, int /*argn*/, char ** /*args*/, int size, int my_rank) { // added size and rank
    ...
    if (_solver_name == "CG_STANDARD") {
        ...
        _pressure_solver = std::make_unique<PCG_SSOR>(
            ...
            _field.p_matrix().num_cols(),
            _field.p_matrix().num_rows());
    } else if (_solver_name == "PCG_SSOR_SERIAL") {
        _pressure_solver = std::make_unique<PCG_SSOR_Serial>(
            eps, itermax,
            _field.p_matrix().num_cols(),
            _field.p_matrix().num_rows());
        ...
        _pressure_solver = std::make_unique<PCG_MG>(
            ...
            _field.p_matrix().num_rows());
#ifdef USE_EIGEN
    } else if (_solver_name == "EIGEN_CG") {
        _pressure_solver = std::make_unique<Eigen_CG>(
            eps, itermax,
            _field.p_matrix().num_cols(),
            _field.p_matrix().num_rows());
