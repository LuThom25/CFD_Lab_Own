...
double Discretization::convection_u(const Matrix<double> &U, const Matrix<double> &V, int i, int j) {
    ...
    double du2_dx = (u_e * u_e - u_w * u_w) * _dx_inv
                  + _gamma * _dx_inv * (
                        std::abs(u_e) * (U(i, j) - U(i + 1, j)) / 2.0
                      - std::abs(u_w) * (U(i - 1, j) - U(i, j)) / 2.0);
    ...
    double duv_dy = (v_n * u_n - v_s * u_s) * _dy_inv
                  + _gamma * _dy_inv * (
                        std::abs(v_n) * (U(i, j) - U(i, j + 1)) / 2.0
                      - std::abs(v_s) * (U(i, j - 1) - U(i, j)) / 2.0);
...
double Discretization::convection_v(const Matrix<double> &U, const Matrix<double> &V, int i, int j) {
    ...
    double duv_dx = (u_e * v_e - u_w * v_w) * _dx_inv
                  + _gamma * _dx_inv * (
                        std::abs(u_e) * (V(i, j) - V(i + 1, j)) / 2.0
                      - std::abs(u_w) * (V(i - 1, j) - V(i, j)) / 2.0);
    ...
    double dv2_dy = (v_n * v_n - v_s * v_s) * _dy_inv
                  + _gamma * _dy_inv * (
                        std::abs(v_n) * (V(i, j) - V(i, j + 1)) / 2.0
                      - std::abs(v_s) * (V(i, j - 1) - V(i, j)) / 2.0);
...
double Discretization::convection_T(const Matrix<double> &T, const Matrix<double> &U, const Matrix<double> &V, int i, int j) {
    // d(uT)/dx terms 
    double uT_dx = (U(i,j)   * interpolate(T, i,   j, 1, 0) - U(i-1,j) * interpolate(T, i-1, j, 1, 0)) * _dx_inv
                 + _gamma * _dx_inv * (std::abs(U(i,j))   * (T(i,j) - T(i+1,j)) / 2.0
                                 - std::abs(U(i-1,j)) * (T(i-1,j) - T(i,j))   / 2.0);

    // d(vT)/dy terms 
    double vT_dy = (V(i,j)   * interpolate(T, i, j,   0, 1) - V(i,j-1) * interpolate(T, i, j-1, 0, 1)) * _dy_inv
                 + _gamma * _dy_inv * (std::abs(V(i,j))   * (T(i,j) - T(i,j+1)) / 2.0
                                 - std::abs(V(i,j-1)) * (T(i,j-1) - T(i,j))   / 2.0);
...
double Discretization::sor_helper(const Matrix<double> &P, int i, int j) {
    ...
    return (P(i + 1, j) + P(i - 1, j)) * _dx2_inv
         + (P(i, j + 1) + P(i, j - 1)) * _dy2_inv;
}
