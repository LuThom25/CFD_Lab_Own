...
double Discretization::convection_T(const Matrix<double> &T, const Matrix<double> &U, const Matrix<double> &V, int i,
    ...
    uT_dx = 0.5 * (U(i, j) * (T(i, j) + T(i + 1, j)) - U(i - 1, j) * (T(i - 1, j) + T(i, j))) / _dx +
            0.5 * _gamma *
                (std::abs(U(i, j)) * (T(i, j) - T(i + 1, j)) - std::abs(U(i - 1, j)) * (T(i - 1, j) - T(i, j))) / _dx;
    vT_dy = 0.5 * (V(i, j) * (T(i, j) + T(i, j + 1)) - V(i, j - 1) * (T(i, j - 1) + T(i, j))) / _dy +
            0.5 * _gamma *
                (std::abs(V(i, j)) * (T(i, j) - T(i, j + 1)) - std::abs(V(i, j - 1)) * (T(i, j - 1) - T(i, j))) / _dy;
...
double Discretization::sor_helper(const Matrix<double> &P, int i, int j) {
    ...
    // P2: fast() for unchecked access; P3: multiply by precomputed reciprocal.
    return (P.fast(i + 1, j) + P.fast(i - 1, j)) * _dx2_inv + (P.fast(i, j + 1) + P.fast(i, j - 1)) * _dy2_inv;
}
